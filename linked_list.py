...
class LinkedList:
    ...
    def add(self, data):
        """
        Adds a new Node containing data at head of the list. Takes constant time
        """
        new_node = Node(data)
    ...
    def search(self, key):
        """
        Search for the first node containing data that matches the key. Return the node or 'None' if not found. Takes O(n) time
        """
        current = self.head
    ...
    def __repr__(self):
        """
        Returns a string representation of the list. Takes linear time
        """
        nodes = []
        current = self.head
